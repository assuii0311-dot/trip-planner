 ...
 * 데이터를 채워야 한다.
 *
 * 처음 쟀을 때 31개였다. 이름만 다른 11개는 등록부를 고쳐 맞췄고,
 * 정말 없던 18개는 손으로 넣었으며(`pipeline/manual-extras.mjs`),
 * 두 개는 데이터에 있는데 `theme: 'food'` 로 분류돼 `isMeal()` 이
 * 볼거리에서 빼고 있었다. **지금은 0개다 — 여기가 지켜야 할 자리다.**
 */
const MISSING_HIGHLIGHTS = 0;
const lost = [];
 ...
 * 49/68/19/22 → 57/85/3/13 이 됐다. **좋아졌으니 기준값도 함께 올린다** —
 * 안 올리면 다음에 예전 자리까지 나빠져도 안 걸린다.
 *
 * 그 뒤 **정답지가 두 번 커졌다.** 등록부 이름 11개를 데이터에 맞춰
 * 150 → 161곳, 없던 장소 18곳을 손으로 넣어 161 → 181곳. 수치는
 * 57/85/3/13 → 55/82/4/14 → 53/80/4/15 로 내려앉았다.
 *
 * **엔진이 나빠진 것이 아니다.** 그때그때 예전 것만 따로 재면 그대로다
 * (150곳 57/85/3/13 · 163곳 55/81/4/14). 새로 들어온 것들이 거리·지구·
 * 산책로처럼 위키백과 항목이 없는 종류라 어렵다(18곳 평균 백분위 21%).
 ...
 */
const BASE = { top10: 53, top25: 80, bottom: 4, mean: 15 };
