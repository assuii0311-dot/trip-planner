...
**나라마다 주소가 다릅니다.** 나라를 고르는 순간 프로그램이 갈라지고,
계획·캐시·홈 화면 아이콘이 나라마다 따로 갑니다.

```
/trip-planner/           나라 고르는 곳
/trip-planner/spain/     스페인  — 도시 60곳
/trip-planner/japan/     일본    — 데이터 수집 중
```

왜 그렇게 했는지와 나라를 붙이는 법은 [docs/23-adding-a-country.md](docs/23-adding-a-country.md).

## 5단계 흐름

1. **기초 정보** — 가고 싶은 도시(그 나라 전체에서 선택), 출발일·도착일, 인원,
...
그 나라를 모르는 사람은 무엇이 좋은지 몰라 고를 수가 없었습니다. 그래서 순서를
...
**일본** 은 아직 수집 전입니다. 접수 페이지에서 카드가 흐려져 있고, 주소로
직접 들어오면 "아직 준비 중" 이 뜹니다 — 망 오류처럼 보이지 않게 한 것입니다.
...
수집 결과는 `app/public/data/<country>/` 로 떨어집니다.
새 나라를 추가하는 다섯 단계는 [docs/23-adding-a-country.md](docs/23-adding-a-country.md) 에 있습니다 —
등록부를 쓰고, 수집하고, 페이지를 만들고, 지도 범위를 넣고, 검사합니다.
**엔진과 화면은 고치지 않습니다.**
...
```bash
cd app
npx tsx scripts/verify.mjs         # 화면 197건
npx tsx scripts/planner-check.mjs  # 엔진 — 반드시 끝나는가, 담은 것이 사라지지 않는가
npx tsx scripts/country-check.mjs  # 나라가 서로 섞이지 않는가
```

전체 목록과 돌리는 법은 [CLAUDE.md](CLAUDE.md) 에 있습니다.
